...
static void gen(Node *node) {
  ...
  switch (node->kind) {
    ...
    case ND_WHILE:
      ...
      return;
    case ND_FOR:
      for (Node *n = node->init; n; n = n->next) gen(n);
      printf(".Lbegin%zu:\n", labelseq);
      if (node->cond) {
        gen(node->cond);
        printf("\tpop rax\n");
      ...
      gen(node->then);
      for (Node *n = node->inc; n; n = n->next) gen(n);
      printf("\tjmp .Lbegin%zu\n", labelseq);
      printf(".Lend%zu:\n", labelseq);
      labelseq++;
      return;
    ...
    case ND_DEREF:
      gen(node->lhs);
      if (node->type->kind != TYPE_ARRAY) load(node->type);
      return;
    case ND_VAR:
      gen_addr(node);
      if (node->type->kind != TYPE_ARRAY) load(node->type);
    ...
    case ND_FUNCALL: {
      ...
      for (size_t i = nargs; i; i--) printf("\tpop %s\n", argreg8[i - 1]);
  ...
  switch (node->kind) {
    ...
    case ND_EQ:
    case ND_NEQ:
      if (node->kind == ND_LT || node->kind == ND_LTE || node->kind == ND_EQ ||
          node->kind == ND_NEQ)
        printf("\tcmp rax, rdi\n");
      else
        printf("\tcmp rdi, rax\n");
...
static void emit_func(Function *functions) {
  ...
  for (Function *func = functions; func; func = func->next) {
    ...
    // 受けた引数をスタックに積む
    size_t i = 0;
    for (VarList *vl = func->params; vl; vl = vl->next) load_arg(vl->var, i++);

    // 関数の実装
    for (Node *node = func->node; node; node = node->next) gen(node);
