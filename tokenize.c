...
Token *expect_type(void) {
  ...
  if (tok->kind != TK_IDENT) error_at(tok->str, "不正な型名です");
  token = token->next;
  return (tok);
}
...
static char *starts_with_reserved(char *p) {
  ...
  for (size_t i = 0; i < sizeof(kw) / sizeof(*kw); i++) {
    ...
    if (l < len) continue;
    if (starts_with(p, kw[i]) && !is_tokstr(p[len])) return (kw[i]);
  }
...
Token *tokenize(void) {
  ...
  while (*p) {
    ...
    if (*p == '"') {
      ...
      while (*p && *p != '"') p++;
      if (!*p) error_at(p, "文字列リテラルが閉じられていません");
      p++;
