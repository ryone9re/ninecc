...
Type *new_type_from_str(char *str) {
  if (!strncmp(str, "char", strlen("char"))) return (new_type(TYPE_CHAR, NULL));
  if (!strncmp(str, "int", strlen("int"))) return (new_type(TYPE_INT, NULL));
  return (NULL);
}
...
void add_type(Program *prog) {
  for (Function *f = prog->functions; f; f = f->next)
    for (Node *n = f->node; n; n = n->next) visit(n);
}
